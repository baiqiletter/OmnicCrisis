...
public class ProjectileStandard : MonoBehaviour
{
    ...
    [Tooltip("Affiliation of the weapon owner")]
    ...
    void OnHit(Vector3 point, Vector3 normal, Collider collider)
    {
        ...
        if (affiliation == 0)
        {
            trueDamage = damage * GameFlowManager.difficulty;
        }
        //Debug.Log("damage: " + trueDamage);
        if (areaOfDamage)
        {
            // area damage
            areaOfDamage.InflictDamageInArea(trueDamage, point, hittableLayers, k_TriggerInteraction, m_ProjectileBase.owner);
        }
        else
        {
            // point damage
            ...
            if (damageable)
            {
                damageable.InflictDamage(trueDamage, false, m_ProjectileBase.owner);
            }
        }
